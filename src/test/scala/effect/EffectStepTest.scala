...
class EffectStepTest extends FunSuite with StepFixtures with Results with Status {
  ...
  test("Promote IO[A] to Step[A]") {
    ...
    whenStepReady(IO.raiseError[Int](new NullPointerException) -| NotFound) { failure =>
  ...
  test("Promote promote FutureBoolean[Boolean] to Step[Unit]") {
    ...
    whenStepReady(IO.pure(false) ?| NotFound){ _ must be(NotFound.asLeft[Unit]) }
  }

  test("Promote Future[Option[A]] to Step[A]") {
    whenStepReady(IO.pure(Option(42)) ?| NotFound) { _ must be(42.asRight[Result])}
    whenStepReady(IO.pure[Option[Int]](None) ?| NotFound){ _ must be(NotFound.asLeft[Int]) }
  }

  test("properly promote Future[Either[B, A]] to Step[A]") {
    whenStepReady(IO.pure(42.asRight[String]) ?| NotFound){ _ must be(42.asRight[Result]) }
