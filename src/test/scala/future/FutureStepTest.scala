...
class FutureStepTest extends FunSuite with StepFixtures with Results with Status {
  ...
  test("Promote Future[A] to Step[A]") {
    ...
    whenStepReady(Future.failed[Int](new NullPointerException) -| NotFound) { failure =>
      failure must be(NotFound.asLeft[Int])
    }
  }

  test("Escalate Future[A] to Step[A]") {
    whenStepReady(Future.successful(42) -| escalate) { _ must be(42.asRight[Result]) }
  ...
  test("Promote Either[B, A] to Step[A]") {
    whenStepReady(Right[String, Int](42) ?| NotFound) { _ must be(42.asRight[Result]) }
  ...
  test("Promote JsResult[A] to Step[A]"){
    ...
    whenStepReady((jsError: JsResult[Int]) ?| (e => BadRequest(JsError.toJson(e)))){
      checkFailingResult[Int](BAD_REQUEST, Json.stringify(JsError.toJson(jsError.errors)))
    }
  }
  ...
  test("Promote Form[A] to Step[A]"){
    ...
    whenStepReady(erroneousForm ?| (f => BadRequest(f.errorsAsJson))){
      checkFailingResult[Int](BAD_REQUEST, erroneousForm.errorsAsJson.toString())
    }
  }
  ...
  test("Promote Try[A] to Step[A]") {
    ...
    whenStepReady((Failure(new Exception("foo")): Try[Int]) ?| (e => BadRequest(e.getMessage))) {
      checkFailingResult[Int](BAD_REQUEST, "foo")
    }
  }

  test("Promote Step[Result] to Result"){
    whenReady(Step.unit(Ok("foo")): Future[Result]){ r => r.header.status must be(OK) }
    whenReady(Step.fail[Result](NotFound): Future[Result]){ r => r.header.status must be(NOT_FOUND) }
  }

  test("Promote promote FutureBoolean[Boolean] to Step[Unit]") {
    whenStepReady(Future.successful(true) ?| NotFound){ _ must be(().asRight[Result])}
    whenStepReady(Future.successful(false) ?| NotFound){ _ must be(NotFound.asLeft[Unit]) }
  }

  test("Promote Future[Option[A]] to Step[A]") {
    whenStepReady(Future.successful(Option(42)) ?| NotFound) { _ must be(42.asRight[Result])}
    whenStepReady(Future.successful[Option[Int]](None) ?| NotFound){ _ must be(NotFound.asLeft[Int]) }
  }

  test("properly promote Future[Either[B, A]] to Step[A]") {
    whenStepReady(Future.successful(42.asRight[String]) ?| NotFound){ _ must be(42.asRight[Result]) }
    whenStepReady(Future.successful("foo".asLeft[Int]) ?| ((s: String) => BadRequest(s))){
      checkFailingResult[Int](BAD_REQUEST, "foo")
    }
  }
}
