...
//    }
  ...
  test("Promote Either[B, A] to Step[A]") {
    ...
    whenStepReady(eitherToStepOps(42.asRight[String]) ?| NotFound) { _ must be(42.asRight[Result]) }
  ...
  test("Promote Boolean to Step[A]") {
    //booleanToStepOps
    //fBooleanToStepOps
    whenStepReady(booleanToStepOps(true) ?| NotFound){ _ must be(().asRight[Result])}
    whenStepReady(booleanToStepOps(false) ?| NotFound){ _ must be(NotFound.asLeft[Unit]) }
  }
